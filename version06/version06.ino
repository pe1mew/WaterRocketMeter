 ...
 * 
 * The sketch is using the Adafruit ADXL345 accelerometer http://www.adafruit.com/product/1231
 * and the Adafruit BMP180 barometric sensor http://www.adafruit.com/product/1603 and the accompanied
 * libraries provided by Adafruit.
 */
...
/**
 ...
 14-3-2015  06        Removed accelerometer axis from struct to save space for more data points
                      Increase of datapints to 1000 at a 100 ms interval
                      Modifications to output data to adapt to the changes
                      Changed data point to time in ms in output data
...
#include <TimerOne.h>            // Library for Timer 1
...
#include <Adafruit_ADXL345_U.h>  // Library for Adafruit ADXL345 accelerometer sensor board
...
volatile bool bIntT1 = false; // Flag for interrupt timer 1 use volatile for shared variables
...
/** \brief function called by interrupt timer 1 
 * Sets flag to remind that interrupt was set
 */
void TimeOut1(void){
  bIntT1 = true;  // Set flag
}
...
void setup() {
  ...
  Timer1.initialize(INTERVAL);      // initilize timer 1
  Timer1.attachInterrupt(TimeOut1); // link interrupt to function blinkLED to run every interrupt
...
void loop() {
  ...
  while (i < SAMPLES){
    ...
    if ( bRunning == true){
      i++;
      // Wait until interrupt will set this flag
      while (bIntT1 == false){
        ; // Wait until interrupt timer 1 has set flag bIntT1 other than false
      }
      bIntT1 = false; // set flag to false to allow interrupt timer 1 to set it again
    }  
  }
  ...
  while(1) {
    ...
    delay(2000);              // wait for a 2 second (1000 ms) before repeat sending data allow to capture data in serial terminal
  };
}
